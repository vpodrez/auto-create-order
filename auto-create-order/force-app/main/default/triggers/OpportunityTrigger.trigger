	...
	if (Trigger.isInsert || Trigger.isUpdate) {
		...
		for(Opportunity opp : opportunities) {
			for (Rule__c rule : rules) {
				if (CompareCondition.compare( String.valueOf(rule.Operator__c), String.valueOf(opp.get(rule.Field_Api_Name__c)), String.valueOf(rule.Field_Value__c), String.valueOf(rule.Field_Api_Type__c))) {
